...
function faceUvs(material, points, normal, group) {
  ...
  if (group.startsWith('hood-') && normal[2] > 0.3) {
    ...
    index = 9; projection = p => [(p[0] + 3.0) / 14.5, (14.0 - p[2]) / 4.5];
    ...
    index = 11; projection = p => [(p[1] + 8.5) / 17.0, (14.5 - p[2]) / 4.5];
  } else if (group === 'front-grille-fascia' && normal[0] > 0.3) {
    index = 12; projection = p => [(p[1] + 8.5) / 17.0, (14.0 - p[2]) / 4.5];
...
const turretMesh = selectMesh(isTurret, [-12.0, 0, 10.2]);
const barrelMesh = selectMesh(isBarrel, [-10.0, 0, 15.2]);
const wheelFLMesh = selectMesh(isWheelFL, [15.0, 9.8, 6.0]);
const wheelFRMesh = selectMesh(isWheelFR, [15.0, -9.8, 6.0]);
const wheelRLMesh = selectMesh(isWheelRL, [-15.0, 9.8, 6.0]);
const wheelRRMesh = selectMesh(isWheelRR, [-15.0, -9.8, 6.0]);
...
const pivots = [
  ...
  ['Bone_TireLF', 0, 15.0, 9.8, 6.0],
  ['Bone_TireRF', 0, 15.0, -9.8, 6.0],
  ['Bone_TireLR', 0, -15.0, 9.8, 6.0],
  ['Bone_TireRR', 0, -15.0, -9.8, 6.0],
  ['Turret', 0, -12.0, 0, 10.2],
  ['Barrel', 5, 2.0, 0, 5.0],
  ['FXMUZZLEFLASH', 6, 24.5, 0, 0.0],
  ['FXTracksL', 0, 0, 9.8, 0],
  ['FXTracksR', 0, 0, -9.8, 0],
