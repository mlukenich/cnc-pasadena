param([string]$ArtDirectory = (Join-Path (Split-Path -Parent $PSScriptRoot) 'src\Art\PM'))
$ErrorActionPreference = 'Stop'
function Assert-True([bool]$Condition, [string]$Message) { if (-not $Condition) { throw "FAIL: $Message" } }

[xml]$model = Get-Content -LiteralPath (Join-Path $ArtDirectory 'PVMudTank_Model.w3x') -Raw
[xml]$declarations = Get-Content -LiteralPath (Join-Path $ArtDirectory 'PVMudTank_Texture.xml') -Raw

$meshes = @($model.AssetDeclaration.W3DMesh)
Assert-True ($meshes.Count -eq 7) "Expected 7 rigid meshes (body, turret, barrel, and 4 wheels), found $($meshes.Count)."

$slots = @{
    DiffuseTexture = 'PVMudTankAtlas';
    NormalMap      = 'PVMudTankNormal';
    SpecMap        = 'PVMudTankSpec';
    RecolorTexture = 'PVMudTankHouse';
}
...
foreach ($id in $slots.Values) {
    Assert-True ($textureIds -contains $id) "Undeclared texture $id in PVMudTank_Texture.xml."
}
...
Assert-True ($spec[$chrome + 2] -ge 60) "Supercharger chrome specular ($($spec[$chrome+2])) is too low."

Write-Host '[PASS] Seven ObjectsGDI materials; four maps; tangent frames; safe paint specular; matte rubber; isolated team recolor.'
