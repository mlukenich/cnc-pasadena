...
function generateMaterialMaps(sourceAtlasPath) {
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      ...
      let bumpScale = 0.15;
      if (cellX === 2 && cellY === 0) bumpScale = 0.40; // tire tread
      if (cellX === 1 && cellY === 1) bumpScale = 0.35; // diamond plate
      if (cellX === 3 && cellY === 1) bumpScale = 0.30; // heat shield
      ...
      if (cellX === 0 && cellY === 0) {
        // Candy Apple Red Paint: strict low-glare limit <= 14
        specR = Math.min(14, Math.round(lum * 0.08));
        specG = 12;
      } else if (cellX === 1 && cellY === 0) {
        // High-Polish Mirror Chrome: >= 60
      ...
      let houseRecolor = 0;
      if (cellX === 0 && cellY === 0) {
        // Base Candy Apple Red gets team recolor
        houseRecolor = Math.round(lum);
      } else if (cellX === 1 && cellY === 2) {
        // Door decal background strip
        if (x % (w / 4) > 10 && y % (h / 4) > 180) houseRecolor = Math.round(lum);
      }
...
function writeMaterialMaps(outputDir) {
  ...
  const sourceAtlas = path.join(texDir, 'mammoth-panels-v1.tga');
  const { atlas, normalPix, specPix, housePix } = generateMaterialMaps(sourceAtlas);

  writeTga32(path.join(outputDir, 'PJMammothAtlas.tga'), atlas.w, atlas.h, atlas.pixels);
  writeTga32(path.join(outputDir, 'PJMammothNormal.tga'), atlas.w, atlas.h, normalPix);
  writeTga32(path.join(outputDir, 'PJMammothSpec.tga'), atlas.w, atlas.h, specPix);
  writeTga32(path.join(outputDir, 'PJMammothHouse.tga'), atlas.w, atlas.h, housePix);
